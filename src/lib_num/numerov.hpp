...
private:
    ...
    struct EvalA{
        EvalA(T h, T x): h{abs(h)*((int)dir)}, h2{h*h}, x{x}{};
    ...
    protected:
        inline virtual T nom(const array<T,2>& y){ return 24.0*y[c]-12.0*y[rl]; };
        inline virtual T denom(){ return 12; };
    ...
    protected:
        inline virtual T nom(const array<T,2>& y){ return Base::nom(y) - Base::h2*(10.0*pow(f_k(Base::x),2)*y[c] + pow(f_k(Base::x_rl),2)*y[rl]); };
        inline virtual T denom(){ return EvalA::denom() + Base::h2*pow(f_k(Base::x_lr),2); };
    };
    ...
    struct EvalD : public T_HOM{
        using Base = T_HOM;
        EvalD(T h, T x): Base(h, x){};

        inline void eval_next(array<T,2>& y){ Base::update_x_neigh(); Base::eval_next(y); }
        static string get_config(){return Base::get_config() + " f_s(...)"; };

    protected:
        inline virtual T nom(const array<T,2>& y){return Base::nom(y) + Base::h2*(f_s(Base::x_rl) + f_s(Base::x_lr) + 10*f_s(Base::x));}
    };
